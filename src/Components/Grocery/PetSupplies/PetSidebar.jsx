...
const PetSidebar = () => {
  ...
  const [isPetsBrandsOpen, setIsPetsBrandsOpen] = useState(true);
  const [isPetsRatingsOpen, setIsPetsRatingsOpen] = useState(true);
  ...
  const togglePets = () => {
    setIsPetsOpen(!isPetsOpen);
  };
  const togglePetsBrands = () => {
    setIsPetsBrandsOpen(!isPetsBrandsOpen);
  };
  const togglePetsRatings = () => {
    setIsPetsRatingsOpen(!isPetsRatingsOpen);
  };
    ...
    <div>
        <div className="category-section">
          <div className="category-box">
            ...
            {isAllCategoriesOpen && (
              <>
                <div className="category-header" onClick={togglePets}>
                  <h4>Pet Supplies</h4>
                  <span className={`dropdown-icon ${isPetsOpen ? 'open' : ''}`}>▼</span>
                </div>
                {isPetsOpen && (
                  <div className="checkbox-group">
                    <label>
                      <input type="checkbox" name="Dog Food" checked={selectedCategories.includes("Dog Food")} onChange={handleCategoryChange} />
                      Dog Food
                    </label>
                    <label>
                      <input type="checkbox" name="Cat Food" checked={selectedCategories.includes("Cat Food")} onChange={handleCategoryChange} />
                      Cat Food
                    </label>
                    <label>
                      <input type="checkbox" name="Pet Grooming" checked={selectedCategories.includes("Pet Grooming")} onChange={handleCategoryChange} />
                      Pet Grooming
                    </label>
                    <label>
                      <input type="checkbox" name="Pet Accessories & Toy" checked={selectedCategories.includes("Pet Accessories & Toy")} onChange={handleCategoryChange} />
                      Pet Accessories & Toy
                    </label>
                    <label>
                      <input type="checkbox" name="Pet Suppliments" checked={selectedCategories.includes("Pet Suppliments")} onChange={handleCategoryChange} />
                      Pet Suppliments
                    </label>
                  </div>
                )}
                <hr className="black-line" />
                <div className="category-header" onClick={togglePetsBrands}>
                  <h4>Brands</h4>
                  <span className={`dropdown-icon ${isPetsBrandsOpen ? 'open' : ''}`}>▼</span>
                </div>
                {isPetsBrandsOpen && (
                  <div className="checkbox-group">
                    <label>
                      <input type="checkbox" name="Pedigree" checked={selectedCategories.includes("Pedigree")} onChange={handleCategoryChange} />
                      Pedigree
                    </label>
                    <label>
                      <input type="checkbox" name="Drools" checked={selectedCategories.includes("Drools")} onChange={handleCategoryChange} />
                      Drools
                    </label>
                    <label>
                      <input type="checkbox" name="Purepet" checked={selectedCategories.includes("Purepet")} onChange={handleCategoryChange} />
                      Purepet
                    </label>
                    <label>
                      <input type="checkbox" name="Wilderness" checked={selectedCategories.includes("Wilderness")} onChange={handleCategoryChange} />
                      Wilderness
                    </label>
                  </div>
                )}
                ...
                <div className="subsection ratings">
                  <div className="category-header" onClick={togglePetsRatings}>
                    <h4>Ratings</h4>
                    <span className={`dropdown-icon ${isPetsRatingsOpen ? 'open' : ''}`}>▼</span>
                  </div>
                  {isPetsRatingsOpen && (
                    <div className="checkbox-group">
                      <label>
