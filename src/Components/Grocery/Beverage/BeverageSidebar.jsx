...
const BeverageSidebar = () => {
    const [selectedCategories, setSelectedCategories] = useState([]);
  const [isAllCategoriesOpen, setIsAllCategoriesOpen] = useState(true);
  const [isBevargeOpen, setIsBevargeOpen] = useState(true);
  const [isBevargeBrandsOpen, setIsBevargeBrandsOpen] = useState(true);
  const [isBevargeRatingsOpen, setIsBevargeRatingsOpen] = useState(true);
  const [isBevargeDiscountsOpen, setIsBevargeDiscountsOpen] = useState(true);
  ...
  const toggleBevarge = () => {
    setIsBevargeOpen(!isBevargeOpen);
  };
  const toggleBevargeBrands = () => {
    setIsBevargeBrandsOpen(!isBevargeBrandsOpen);
  };
  const toggleBevargeRatings = () => {
    setIsBevargeRatingsOpen(!isBevargeRatingsOpen);
  };
    ...
    <div>
        <div className="category-section">
          <div className="category-box">
            ...
            {isAllCategoriesOpen && (
              <>
                <div className="category-header" onClick={toggleBevarge}>
                  <h4> Bevarge</h4>
                  <span className={`dropdown-icon ${isBevargeOpen ? 'open' : ''}`}>▼</span>
                </div>
                {isBevargeOpen && (
                ...
                <div className="category-header" onClick={toggleBevargeBrands}>
                  <h4>Brands</h4>
                  <span className={`dropdown-icon ${isBevargeBrandsOpen ? 'open' : ''}`}>▼</span>
                </div>
                {isBevargeBrandsOpen && (
                  <div className="checkbox-group">
                    <label>
                ...
                <div className="section-gap"></div>
                  <div className="subsection ratings">
                    <div className="category-header" onClick={toggleBevargeRatings}>
                      <h4>Ratings</h4>
                      <span className={`dropdown-icon ${isBevargeRatingsOpen ? 'open' : ''}`}>▼</span>
                      </div>
                      {isBevargeRatingsOpen && (
                        <div className="checkbox-group">
                          <label>
                            <input
                            type="checkbox"
                ...
                <hr className="black-line" />
                <div className="category-header" onClick={() => toggleSection(setIsBevargeDiscountsOpen)}>
                  <h4>Discount</h4>
                  <span className={`dropdown-icon ${isBevargeDiscountsOpen ? 'open' : ''}`}>▼</span>
                </div>
                {isBevargeDiscountsOpen && (
                  <div className="checkbox-group">
                  ...
                  <div>
                    <div className="price-range-container">
                      ...
                      <input type="range"min="100"max="500"
                        value={priceRange}onChange={handlePriceChange}className="price-range"
                      />
                      <span>500</span>
                    </div>
...
export default BeverageSidebar